...
void Foam::kineticTheoryModel::solve(const volTensorField& gradU1t)
{
    ...
    volScalarField k1 = U1_.mesh().lookupObject<volScalarField> ("k1");
    dimensionedScalar alphaSmall("alphaSmall", dimless, 1.0e-6);

    // - Declare local refs for KT equation
    const volScalarField& alphaRho1_ = U1_.mesh().lookupObject<volScalarField> ("alphaRho1");
    surfaceScalarField phi(rho1_*phi1_*fvc::interpolate(alpha1_+ alphaSmall));
